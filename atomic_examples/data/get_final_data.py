...
year_months = ['2022-04']#['2022-01', '2022-02', '2022-03']
...
for year_month in year_months:
    ...
    #delete PU_DO that occur less= than 10 times. From 6966 distinct categories (and 62495 rows) to 809 distinc categories (and 48672 rows) for green_tripdata_2022-01
    pu_do = df['PU_DO'].value_counts() #series
    print('distinct pu_do \n',pu_do.shape)
    print('total df rows \n',pu_do.sum()) # equal df.shape[0]
    print('distinct pu_do>10 \n',pu_do[pu_do>10].shape)
    print('total rows pu_do>10 \n',pu_do[pu_do>10].sum())

    pu_do_count = pu_do.reset_index().rename(columns={'PU_DO':'count', 'index':'PU_DO'})
    print(pu_do_count.head())
    pu_do_count_filtered = pu_do_count.loc[pu_do_count['count']>10].drop(columns=['count'],axis=1)
    df_filtered = pd.merge(left=df,right=pu_do_count_filtered, on='PU_DO').drop(columns=['PU_DO'],axis=1)

    assert df_filtered.shape[0]==pu_do[pu_do>10].sum()
